...
- [x] Rspack for fast TypeScript compilation and bundling
...
- [x] DDD patterns (Entities, Value Objects, Aggregates, Repositories)
- [x] CQRS pattern with separate read/write repositories
- [x] Zod for runtime validation
- [x] Result pattern for error handling

## Development Setup

This project uses modern tooling for fast development and builds:

### Prerequisites

- **Bun** (>= 1.0.0): Fast JavaScript runtime and package manager
- **Node.js** (>= 18.12.0): Required for some tooling compatibility
...
# The .env.local file is ignored by git and should contain your actual values
```
...
- **APP_PORT**: Port for the HTTP server (default: 3001)
- **APP_ENV**: Application environment (`development` | `test` | `production`)
- **DB_HOST**: PostgreSQL database host (default: localhost)
- **DB_USER**: PostgreSQL database username
- **DB_PASS**: PostgreSQL database password
- **DB_DATABASE**: Main database name (default: ddd)
- **DB_DATABASE_TEST**: Test database name (default: ddd_test)
- **DB_PORT**: PostgreSQL database port (default: 5432)
- **DATABASE_URL**: Alternative full connection URL (used by drizzle-kit)
- **NO_COLOR**: Set to disable colored console output
- **NODE_ENV**: Build environment (used by Rspack)
...
# This uses Rspack to watch for file changes and rebuild automatically
```
...
# This compiles TypeScript to JavaScript using Rspack with SWC loader
# Output is optimized and minified for production
```
...
# Runs the compiled CommonJS output from dist/main.cjs
```
...
- **Rspack**: High-performance bundler written in Rust, much faster than webpack/tsc direct compilation
- **Bun**: Significantly faster package manager and runtime compared to npm/Node.js
- **SWC**: Fast TypeScript/JavaScript compiler used by Rspack for instant compilation
- **Hot Reloading**: Development server with automatic rebuilds on file changes

The build process transforms TypeScript source code into optimized CommonJS modules ready for production deployment.
